...
"""
This is a slightly edited version of yolov5_learner.py developed by OpenDR. I have edited a few lines of the original code in order to use my
own custom model located on my own PC and to set the confidence threshold for the model manually.
...
The original version of this script can be found from OpenDR's GitHub page: https://github.com/opendr-eu/opendr.
...
###   My changes < ###
# Importing the path to my custom model and the confidence threshold value
from custom_imports.constants import LOCAL_MODEL_PATH
from custom_imports.constants import CONFIDENCE
### > My changes   ###
...
class YOLOv5DetectorLearner(Learner):
    ...
    def __init__(self, model_name, path=None, device='cuda', temp_path='.', force_reload=False):
        ...
        # Getting a generic model
            ...
            ###   My changes < ###
                # Using a custom model stored locally in the PC if the 'path' is specified
                self.model = torch.hub.load(LOCAL_MODEL_PATH, 'custom', path=path,
                                            source="local", force_reload=force_reload)
            # Setting the confidence threshold that is specified in 'custom_imports/constants.py'
