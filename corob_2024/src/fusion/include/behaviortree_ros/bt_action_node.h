...
protected:
  ...
  BT::ActionNodeBase(name, conf), node_(nh)
  {
    ...
    feedback_ = boost::make_shared<const FeedbackType>(); 
  }
...
public:
  ...
  using FeedbackConstPtr = boost::shared_ptr<const FeedbackType>; 
  ...
  BT::NodeStatus tick() override
  {
    ...
    if (status() == BT::NodeStatus::IDLE) {
      ...
      action_client_->sendGoal(goal, 
                              typename ActionClientType::SimpleDoneCallback(), 
                              typename ActionClientType::SimpleActiveCallback(),
                              boost::bind(&RosActionNode::onFeedback, this, _1));
    }
